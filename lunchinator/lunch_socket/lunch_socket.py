...
class lunchSocket(object):    
    ...
    def sendto(self, msg, ip, disable_extended=False):
        ...
        if len(msg) > self._max_msg_length:                     
            if disable_extended:
                getLogger().warning("Message to peer %s is too long and should be compressed/split, " + \
        ...
        try:        
            if disable_extended:
                ...
                getLogger().debug("Sending as extended Message")
                xmsg = extMessageOutgoing(msg, self._max_msg_length)
                for f in xmsg.getFragments():
                    self._s.sendto(f, (ip, self._port))
        except socket.error as e:
            if e.errno in [64,65]:
                getLogger().debug("lunch_socket: Removing IP because host is down or there is no route")
    ...
    def broadcast(self, msg):
        try:
            ...
            if len(msg) > self.LEGACY_MAX_LEN:
                getLogger().debug("Broadcasting as extended Message")
                xmsg = extMessageOutgoing(msg, self._max_msg_length)
                for f in xmsg.getFragments():
            ...
            getLogger().exception("Problem while broadcasting")
